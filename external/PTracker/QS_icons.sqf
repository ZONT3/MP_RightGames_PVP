...
_QS_ST_map_enableUnitIcons = true;							// Включить маркеры юнитов и ТС
_QS_ST_gps_enableUnitIcons = true;							// Включить маркеры юнитов и ТС в GPS
_QS_ST_enableGroupIcons = true;								// Включить маркеры группы на карте, ХУДе и ГПС

// Админ

_QS_ST_admin = true;										// Включить отображение всех юнитов, если вошел как админ
_QS_ST_showAll = 0;											// Откладка в буфер -  0 = выключить. 1 = Показать всех юнитов и технику. 2 = Показать все объекты, юниты и тс

// setfriend типа, но в метках

_QS_ST_friendlySides_Dynamic = false;						// Установите значение TRUE, чтобы разрешить динамические изменения альянсов фракций (например, AAF не всегда может быть лояльным к НАТО) и отображать их на карте
...
_QS_ST_showMOS = false;											// показывать класс ТС, роль и т.д.
...
QS_ST_STR_text1 = 'Клик, чтобы показать информацию';				// STRING. Text shown when a player passes Mouse over Group leader (only if _QS_ST_groupInteractiveIcons = TRUE;)
QS_ST_STR_text2 = 'Эта группа не в вашей фракции';			// STRING. Text shown when a player clicks on a Group Icon of other faction. (only if _QS_ST_groupInteractiveIcons = TRUE;)
